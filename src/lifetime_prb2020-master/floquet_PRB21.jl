...
#[2.0,3.5,5.5,8.25] This is the data set which is most often used
##################################################################################################
##################################################################################################
##################################################################################################
...
#=Makes tri-diagonal for interacting case =#

function makeMInt(L,bs)
    tmp = zeros(L+1,L+1)
    for i in 1:length(bs)
        tmp[i,i+1] = bs[i]
        tmp[i+1,i] = bs[i]
...
function free_W_edge_modes()
	...
	evalues[:,ti] = imag.(log.(u[:]))
	println("EV="," " , evalues[:,ti])
        # plotting
    ...
    fig.show()
    #save("data21/Wfree.jld", "Tps",Tps,"evalues",evalues)
end
...
function interacting_W_edge_modes()
     Ls = [4,8]	 #lengths used for Fig 8
    #Ls = [4,10] # lengths used for spectrum of W
    ...
    #Ns = [6,12,20] #This was used for spectrum of W. 
    t_len = 5000 # length of ainf, in periods. OLD version was 5000
    # store results here
    evaluesInt = zeros(Ns[3],length(Ts))
    ...
    for (Li,L) in enumerate(Ls) # step through system sizes
		...
		if Li==2 && Ni==3
		uW, vW = eigen(W) 
		evaluesInt[:,Ti] = imag.(log.(uW[:]))
		println("EV=", evaluesInt[:,Ti])
		end
    ...
    fig,ax = plt.subplots(length(Ls),length(Ts),
                          sharey = true,sharex = true, figsize = [9,5])
    fig.subplots_adjust(wspace = 0.0,hspace = 0.0)
    for (Li,L) in enumerate(Ls)
        for (Ti,T) in enumerate(Ts)
            ax[Li,Ti].plot(1:t_len,data_ainf1[:,Ti,Li],"o", ms=5, label = "ED")
            for (Ni,N) in enumerate(Ns)
	    	if Ni== 3
                ...
                ax[Li,Ti].set_xlabel("\$n\$")
                ax[Li,Ti].set_xscale("log")
            end
        end
        ax[Li,1].set_ylabel("\$A_{\\infty}(n T) \$")
    end
    ...
    #ax[2,1].legend()
    ax[1,1].text(0.45, 0.1, "\$L=4, T=2.0\$", transform=ax[1,1].transAxes)
    ax[1,2].text(0.45, 0.1, "\$L=4, T=3.5\$", transform=ax[1,2].transAxes)
    ax[1,3].text(0.45, 0.1, "\$L=4, T=5.5\$", transform=ax[1,3].transAxes)
    ax[1,4].text(0.45, 0.09, "\$L=4, T=8.25\$", transform=ax[1,4].transAxes)
    ax[2,1].text(0.45, 0.1, "\$L=8, T=2.0\$", transform=ax[2,1].transAxes)
    ax[2,2].text(0.45, 0.1, "\$L=8, T=3.5\$", transform=ax[2,2].transAxes)
    ax[2,3].text(0.45, 0.1, "\$L=8, T=5.5\$", transform=ax[2,3].transAxes)
    ax[2,4].text(0.45, 0.9, "\$L=8, T=8.25\$", transform=ax[2,4].transAxes)
    fig.savefig("fig8.png",bbox_inches = "tight",dpi = 800)
    fig.show()
    #save("data21/WintJz0p05N20L10.jld", "Tps",Tps,"evaluesInt",evaluesInt)
end

#= This one is used to show explicit N dependence =#

function interacting_W_edge_modes2()
     Ls = [4]	 #lengths used for Fig 9
    ...
    for (Li,L) in enumerate(Ls)
        for (Ti,T) in enumerate(Ts)
            ax.plot(1:t_len,data_ainf1[:,Ti,Li],"o", ms=5, label = "ED")
            for (Ni,N) in enumerate(Ns)
                ...
                ax.plot(1:t_len,tmp_plt,
                               "^",
                               ms = 2.0, 
                               label = "\$W\$")
	        		       
               
                    ax.plot(1:t_len,tmp_plt2,
                                   "-|",
                                   lw = 2.0, ms=2,
                                   label = "\$d_{1,0}\$")
                    ax.plot(1:t_len,tmp_plt3,
                                   "-^",
                                   lw = 2.0, ms =2,
                                   label = "\$d_{1,\\pi}\$")

                    ax.plot(1:t_len,tmp_plt2b,
                                   "-",
                                   lw = 2.0, ms=2,
                                   label = "\$d_{2,0}\$")
                    ax.plot(1:t_len,tmp_plt3b,
                                   "-",
                                   lw = 2.0, ms=2,
                                   label = "\$d_{2,\\pi}\$")
    ...
    ax.text(0.45, 0.1, "\$L=4, T=2.0\$", transform=ax.transAxes)
    fig.savefig("fig9.png",bbox_inches = "tight",dpi = 800)
    fig.show()
    #save("data21/WintJz0p05N20L10.jld", "Tps",Tps,"evaluesInt",evaluesInt)
end
...
#    Ts = [.5,1.5,3.6,5.,8.25]
	...
	data_wavef2L[Ti,:] = abs.(v[:,2*L]).^2 # Store the k=2*L eigenvector
        data_bs[Ti,:] = bs
	#println("WF=",data_wavef2L[Ti,:])
    end
    # rest is plotting
    fig,ax = plt.subplots(2,length(Ts),sharey = "row", figsize = [6.8,5.])
    fig.subplots_adjust(wspace = 0.0, hspace=0.3)
    for (Ti,T) in enumerate(Ts)
        ax[1,Ti].plot(data_bs[Ti,:],"-o", ms=3.0, lw=1.0)
        ax[2,Ti].plot(data_engs[Ti,:],"-o", ms=3.0, lw=1.0)
    ...
    ax[1,1].text(0.1, 0.1, "\$T=2.0\$", transform=ax[1,1].transAxes)
    ax[1,2].text(1.1, 0.05, "\$T=3.5\$", transform=ax[1,1].transAxes)
    ax[1,3].text(2.1, 0.1, "\$T=5.5\$", transform=ax[1,1].transAxes)
    ax[1,4].text(3.1, 0.1, "\$T=8.25\$", transform=ax[1,1].transAxes)
    ax[1,1].set_ylabel("\$ b_n \$")
    ax[2,1].set_ylabel("spectrum of \$H_K\$")
    fig.savefig("fig2.png",bbox_inches = "tight",dpi = 800)
    fig.show()

    #= WARNING: WHEN DOING SINGLE ROW ONLY CHANGE ax[1,Ti] to ax[Ti]. Annoying bug in mathplotlib. =#
    fig,ax = plt.subplots(2,length(Ts),sharey = "row", sharex= true, figsize=[3.4,5])
    fig.subplots_adjust(wspace = 0.0,hspace=0.0)
    for (Ti,T) in enumerate(Ts)
        ax[2,Ti].plot(data_wavef1[Ti,:],"-o", ms=2)
        ax[1,Ti].plot(data_wavefL[Ti,:],"-+", ms=2)
	ax[1,Ti].plot(data_wavefLp1[Ti,:],"-o", ms=2)
        ax[2,Ti].plot(data_wavef2L[Ti,:],"-+",ms=2)
        #ax[1,Ti].set_xlabel("\$n\$", fontsize=12)
	ax[2,Ti].set_xlabel("\$n\$")
    end
    ax[1,1].text(0.1, 0.5, "\$T=2.0\$", transform=ax[1,1].transAxes)
    ax[1,2].text(1.1, 0.5, "\$T=3.5\$", transform=ax[1,1].transAxes)
    ax[1,3].text(2.1, 0.5, "\$T=5.5\$", transform=ax[1,1].transAxes)
    ax[1,4].text(3.1, 0.5, "\$T=8.25\$", transform=ax[1,1].transAxes)
    ax[1,1].set_ylabel("\$|\\psi_0(n)|^2\$")
    ax[2,1].set_ylabel("\$|\\psi_\\pi(n)|^2\$")
...
function floquet_ham_interacting_lanczos()
    ...
    g = .3
    jx = 1.0
    Ts = [.5,1.5,3.6,5.,8.25] #When taking data from top, use these parameters
    #Ts= [2.0,3.5,5.5,8.25]
...
function ainf_example_HF()
    ...
    Ts = 3/2 .* [2.0,3.5,5.5,8.25]
    # times to calculate
    times = ceil.(10 .^ range(0.0,6.0,step =.2)) # Logarithmically spaced point with only whole periods
    ...
    for (Ti,T) in enumerate(Ts)
        ...
        for (ti,time) in enumerate(times)
            ...
            push!(ainf,real.(1/(2^L)*tr(D'*sigxv*D*sigxv)))
        end
        push!(data,ainf)
    end
    # rest is plotting
    fig,ax = plt.subplots(1,1, figsize=[3.4,5])
    #ax.text(0.0, 0.25, "\$L=8\$", transform=ax.transAxes, fontsize=12)
    ax.text(0.03, 0.26, "\$L=8\$", transform=ax.transAxes)
    markers = ["-o","-s","-^","-*","->"]
    for (Ti,T) in enumerate(Ts)
        ax.plot(times,data[Ti],markers[Ti],ms= 5.0,lw = 1.5,color = "C$(Ti-1)",label = "\$T = $((2/3)*T) \$") #Here reverting back to free period
    end
    ax.set_xscale("log")
    ax.set_xlabel("\$ n \$") # in bracket as fontsize = 16 to vary fontsize 
    ax.set_ylabel("\$ A_\\infty(n T) \$")
    ax.legend(loc = "upper center",
		  fontsize = "small",
		  framealpha = 1.0,
...
function free_lanczos()
    ...
    # rest is plotting
    fig,ax = plt.subplots(3,length(Ts),sharey = "row", figsize=[6.8,6.0])
    fig.subplots_adjust(wspace = 0,hspace=0.4)
    for (Ti,T) in enumerate(Ts)
        ax[1,Ti].plot(Bs1[Ti],"-o",label = "spin\$_1\$")
      ax[1,Ti].plot(Bs2[Ti],"-x",ms = 10,label = "free")
      ax[1,Ti].plot(Bs1b[Ti],"--o",label = "spin\$_2\$")
      ax[2,Ti].plot(engs1[Ti],"-o",label = "spin\$_1\$")
      ax[2,Ti].plot(engs2[Ti],"-x",label = "free")
      ax[2,Ti].plot(engs1b[Ti],"--o",label = "spin\$_2\$")
      ax[2,Ti].axhline(y = pi,alpha = .2,color = "red")
      ax[2,Ti].axhline(y = -pi,alpha = .2,color = "red")
      ax[3,Ti].plot(times ,ainfs1[Ti],"-o",label = "spin\$_1\$")
      ax[3,Ti].plot(times ,ainfs2[Ti],"-x",label = "free")
      ax[3,Ti].plot(times ,ainfs1b[Ti],"-s",label = "spin\$_2\$")
      ax[3,Ti].plot(times , ainfs0[Ti],"-^",label = "ED")
        ax[3,Ti].set_xlabel("\$n \$")
        ax[2,Ti].set_xlabel("index")
        ax[1,Ti].set_xlabel("\$n \$")
        ax[2,Ti].text(.42,.05, fontsize = "small", "\$T=$(T)\$", transform = ax[2,Ti].transAxes)
    end
    ax[1,3].legend(fontsize="small", ncol=3)
    ax[3,1].legend(fontsize = "small")
    ax[3,1].set_ylabel("\$A_\\infty(n T)\$")
    ax[2,1].set_ylabel("spectrum of \$ H_K \$")
    ax[1,1].set_ylabel("\$b_n \$")
    fig.savefig("fig4.png",bbox_inches = "tight",dpi = 800)
    fig.show()
end
...
for interacting case. Loops through T
=#

function gen_bn_lanczos_HF_T()
    Ls = [6,8,10] # system sizes
    # system parameters
    g = .3
    jz = 0.001
    jx = 1.0
    Ts = 3/2. *[0.5,2.0,3.5,5.5,8.25]
    N = 60 # number of states to calculate
    Bs = zeros(N-1,length(Ls),length(Ts))
  ...
  #save("data21/test.jld", "Ls", Ls,"Ts",Ts,"Bs",Bs)
    
    # plotting: WARNING SOMETIMES USING 5 DIFFEREN T's. SO PLOT BELOW MAY NEED TO BE CHANGED
    fig,ax = plt.subplots(2,2) 
    for (Li,L) in enumerate(Ls)
        ax[1,1].plot(abs.(Bs[:,Li,1]),"-o",label = "\$ L = $L\$")
	ax[1,2].plot(abs.(Bs[:,Li,2]),"-o",label = "\$ L = $L\$")
	ax[2,1].plot(abs.(Bs[:,Li,3]),"-o",label = "\$ L = $L\$")
	ax[2,2].plot(abs.(Bs[:,Li,4]),"-o",label = "\$ L = $L\$")
    end
    ax[1,1].legend()
    ax[1,2].legend()
    ax[2,1].legend()
...
#= Reads from file and plots for different T
=#

function read_plotbnT()

       #data = load("data21/bnTjz0p001_L6-8-10.jld")
       data = load("data21/bnTjz0p01_L6-8-10.jld")
       Ls = data["Ls"]
       Ts = data["Ts"]
       Bs = data["Bs"]
       Lb = length(Bs[:,1,1]) # Number of bn. Assumes same number of bn for all Li and Ti
       data_engs = zeros(length(Ts), Lb+1)
       for (Ti,T) in enumerate(Ts)
	bs = Bs[:,length(Ls),Ti] #Consider longest length
	println("T = ", 2/3*T, " and " ,"Number of bn = ", Lb)
	println("bs = ", bs[:])
     
	
        # build Krylov Hamiltonian and diagonalize, get quasienergy
        HK = makeMInt(Lb,bs) # plop bns into tri-diagonal matrix
        u,v = eigen(HK) # diagonalize
        data_engs[Ti,:] = u[:]
	println("EV = ", u[:])
	end
       ...
       #= WARNING NOT ALL THE Ti ARE BEING PLOTTED =# 
       fig,ax = plt.subplots(2,2, figsize=[6.4,5])
       fig.subplots_adjust(hspace = 0.25)
       for (Li,L) in enumerate(Ls)
        ax[1,1].plot(abs.(Bs[:,Li,1]), markers[Li], ms= 3.,lw = .5, label = "\$ L = $L\$")
	ax[1,2].plot(abs.(Bs[:,Li,2]), markers[Li], ms= 3.,lw = .5, label = "\$ L = $L\$")
	ax[2,1].plot(abs.(Bs[:,Li,3]), markers[Li], ms= 3.,lw = .5, label = "\$ L = $L\$")
	ax[2,2].plot(abs.(Bs[:,Li,5]), markers[Li], ms= 3.,lw = .5, label = "\$ L = $L\$")
    end
    ax[1,2].axhline(y =  pi,color = "black",alpha = .2)
    ax[2,1].axhline(y =  pi,color = "black",alpha = .2)
    ax[2,2].axhline(y =  pi,color = "black",alpha = .2)
    ax[1,1].text(0.75, 0.9, "\$T=0.5\$", transform=ax[1,1].transAxes)
    ax[1,2].text(0.75, 0.9, "\$T=2.0\$", transform=ax[1,2].transAxes)
    ax[2,1].text(0.75, 0.9, "\$T=3.5\$", transform=ax[2,1].transAxes)
    ax[2,2].text(0.75, 0.9, "\$T=8.25\$", transform=ax[2,2].transAxes)
    ax[1,1].set_ylabel("\$b_n\$")
    ax[1,2].set_ylabel("\$b_n\$")
    ax[2,1].set_ylabel("\$b_n\$")
    ax[2,2].set_ylabel("\$b_n\$")
    ax[1,1].set_xlabel("\$n\$")
    ax[1,2].set_xlabel("\$n\$")
    ax[2,1].set_xlabel("\$n\$")
    ax[2,2].set_xlabel("\$n\$")
    ...
    #ax[2,1].legend()
    #ax[2,2].legend()
    fig.savefig("fig5.png",bbox_inches = "tight",dpi = 800)
    fig.show()

    fig,ax = plt.subplots(1,4,sharey = "row", figsize = (6.4,5))
    fig.subplots_adjust(hspace = 0.01)
        ax[1].plot(data_engs[1,:],"-o", ms=3.0, lw=1.0)
        ax[2].plot(data_engs[2,:],"-o", ms=3.0, lw=1.0)
	ax[3].plot(data_engs[3,:],"-o", ms=3.0, lw=1.0)
        ax[4].plot(data_engs[5,:],"-o", ms=3.0, lw=1.0)

	
	ax[1].axhline(y = -pi,color = "red",alpha = .5)
        ax[1].axhline(y =  pi,color = "red",alpha = .5)
        ax[1].axhline(y =  0 ,color = "red",alpha = .5)

	ax[2].axhline(y = -pi,color = "red",alpha = .5)
        ax[2].axhline(y =  pi,color = "red",alpha = .5)
        ax[2].axhline(y =  0 ,color = "red",alpha = .5)

	ax[3].axhline(y = -pi,color = "red",alpha = .5)
        ax[3].axhline(y =  pi,color = "red",alpha = .5)
        ax[3].axhline(y =  0 ,color = "red",alpha = .5)

	ax[4].axhline(y = -pi,color = "red",alpha = .5)
        ax[4].axhline(y =  pi,color = "red",alpha = .5)
        ax[4].axhline(y =  0 ,color = "red",alpha = .5)

	ax[1].set_ylabel("spectrum of \$H_K\$")
	#ax[2].set_ylabel("spectrum of \$H_K\$")
	#ax[3].set_ylabel("spectrum of \$H_K\$")
	#ax[4].set_ylabel("spectrum of \$H_K\$")

	ax[1].set_xlabel("index")
	ax[2].set_xlabel("index")
	ax[3].set_xlabel("index")
	ax[4].set_xlabel("index")
    	ax[1].text(0.1, 0.1, "\$T=0.5\$", transform=ax[1].transAxes)
    	ax[2].text(0.22, 0.1, "\$T=2.0\$", transform=ax[2].transAxes)
    	ax[3].text(0.3, 0.1, "\$T=3.5\$", transform=ax[3].transAxes)
    	ax[4].text(0.4, 0.1, "\$T=8.25\$", transform=ax[4].transAxes)
    	fig.savefig("fig6.png",bbox_inches = "tight",dpi = 800)
    fig.show()
...
function gen_bn_lanczos_HF()
    #Ls = [10,12] # system sizes
    Ls = [10,12]
    # system parameters
    g = .3    
    jzs = [0.001,0.00125,0.0015,0.002,0.003,0.005,0.01,0.05]
    jx = 1.0
    T = 3/2 * 8.25
    println("T = $(2/3*T)")
    N = 40 # number of states to calculate. QUESTION: APPEARS TO BE N-1
    Bs = zeros(N-1,length(Ls),length(jzs))
    # step through system sizes
    ...
    save("data21/test.jld", "Ls", Ls,"jzs",jzs,"Bs",Bs)
    ...
    for (jzi,jz) in enumerate(jzs)
	for (Li,L) in enumerate(Ls)
	    ax[jzi].plot(Bs[:,Li,jzi],"-o",label = "\$ L = $L\$")
	end
    end
    for (jzi,jz) in enumerate(jzs)
     ax[jzi].legend()
...
function simple_ainfFL2()
    ...
    Ls = [6,8,10] # different lengths. WARNING, for L=14 do not do more than 100 points
    # Hamiltonian parameters
    jx = 1.0
    g = .3
    jz = 0.02
    ...
    t_len = collect(range(1,5000,step=1)) # linearly space points
    ...
    data_ainf1 = zeros(length(t_len),length(Ls)) # exact ainf
		  ...
		  save("data21/test.jld", "Ls", Ls,"times",t_len,"Ainfs",data_ainf1)
    		...
    		fig,ax = plt.subplots(1,1)
		       for (Li,L) in enumerate(Ls)
     	    	       ax.plot(t_len,data_ainf1[:,Li], markers[Li], label = "\$ L = $(Ls[Li])\$")
		       end
     		       ax.set_xscale("log")
		       ax.legend()
    		       plt.show()
...
#Reads data, plots it. Comment out when needed
...
# Reads EV of W and plots them

function read_plotW()

	 #save("data21/Wfree.jld", "Tps",Tps,"evalues",evalues)
	 #save("data21/WintJz0p05.jld", "Tps",Tps,"evaluesInt",evaluesInt)
	 #save("data21/WintJz0p05N22.jld", "Tps",Tps,"evaluesInt",evaluesInt)

	 data1 = load("data21/Wfree.jld")
	 data2 = load("data21/WintJz0p05N20L10.jld")
       	 Tps = data1["Tps"]
	 evalues = data1["evalues"]
	 evaluesInt = data2["evaluesInt"]
       	      fig,ax = plt.subplots(2,length(Tps), sharex=true, sharey= true, figsize=[3.4,5])
	      fig.subplots_adjust(wspace = 0.0,hspace = 0.0)
    	    	   for (ti,T) in enumerate(Tps)
		   ax[1,ti].axhline(y = 0, color = "red", alpha = .5)
		   ax[2,ti].axhline(y = 0, color = "red", alpha = .5)
		   ax[1,ti].axhline(y = -pi,color = "red",alpha = .5)
        	   ax[1,ti].axhline(y =  pi,color = "red",alpha = .5)
		   ax[2,ti].axhline(y = -pi,color = "red",alpha = .5)
        	   ax[2,ti].axhline(y =  pi,color = "red",alpha = .5)
		   ax[1,ti].plot(evalues[:,ti], ".")
		   ax[1,ti].text(0.1, 0.52, "\$T=$(T)\$", transform=ax[1,ti].transAxes)
		   ax[1,ti].set_xlabel("index")  
		   #ax[1,ti].legend()
     	    	   ax[2,ti].plot(evaluesInt[:,ti], ".")
		   ax[2,ti].text(0.06, 0.52, "\$T=$(T)\$", transform=ax[2,ti].transAxes)
		   ax[2,ti].set_xlabel("index")
		   
		   #ax[2,ti].legend()
		   end
		   ax[1,1].set_ylabel("spectrum free \$ i \\ln(W)\$")
		   ax[2,1].set_ylabel("spectrum interacting \$ i \\ln(W)\$")
		   fig.savefig("fig7.png",bbox_inches = "tight",dpi = 800)
    			plt.show()
...
function read_plotbn()
	...
	for (Li,L) in enumerate(Ls)
	    #ax[jzi].plot(Bs[:,Li,jzi],"-o",label = "\$ L = $L\$")
	    ax[jzi].plot(Bs[:,Li,jzi],"-o",label = "\$ J_z = $jz\$")
	end
    	end
    	for (jzi,jz) in enumerate(jzs)
     	ax[jzi].legend()
	...
	fig,ax = plt.subplots(length(jzs),1)
        for (jzi,jz) in enumerate(jzs)
	    ax[jzi].plot(M0[:,jzi],"-x",label = "\$ J_z = $jz\$")
	end
    	...
    	fig.show()
			...
			fig,ax = plt.subplots(1,1)
			ax.plot(1. ./jzs,Mend[:],"-o",label = "M0[11]")
   	                plt.legend()
                        plt.show()
...
s1=-0.001
H = [0 -im*s1 0 0 0 0; im*s1 0 -im 0 0 0;0 im 0 -im*s1 0 0; 0 0 im*s1 0 -im 0; 0 0 0 im 0 -im*s1;0 0 0 0 im*s1 0]
 u, v = eigen(H)
 println("Eigenvalues = ", u[:])
 println("Eigenvectors = ", v[3,:], " and ", v[4,:]) 
...
# @timev ainf_example_HF() # ainf example using HF, so micromotion will be off. [pap1-PLOT1]

# @timev floquet_ham_free_lanczos() # b_ns and Krylov Hamiltonian spectrum, quick to run [pap1-PLOT2+PLOT3]

# @timev free_W_edge_modes()
#=makes W and finds approximate edge modes for free case, quick to run. GENERATES DATA FOR pap1-PLOT7 (upper-panel)=#
...
# @timev interacting_W_edge_modes()
#= finds approximate W edge modes, time evolves, quick to run [pap1-PLOT8] AND GENERATES DATA FOR pap1-PLOT7 (lower-panel)=#

#    @timev interacting_W_edge_modes2() #[pap1-PLOT9]#
...
# @timev free_lanczos() # studying difference between b_n of majorana and spin basis Lanczos on HF PAPER1-PLOT4

#  @timev gen_bn_lanczos_HF_T() #Does not loop through Jz, but loops through T: DATA FOR PAPER1-PLOT5

#  @timev read_plotbnT() #reads saved data for bn, manilpulates and plots it #[pap1-PLOT5+PLOT6]. Loops through T

# @timev gen_bn_lanczos_HF() # Loops through Jz. GENERATES THE DATA USED IN INTERACTING PAPER.
...
#  @timev simple_ainfFL2() #run this one for the paper

#   @timev read_plot() #reads saved data for Ainf, manilpulates and plots it 

#   @timev read_plotbn() #reads saved data for bn, manilpulates and plots it 

#    @timev read_plotW() # pap1-PLOT7
